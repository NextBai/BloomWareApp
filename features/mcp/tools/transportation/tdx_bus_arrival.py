...
from .tdx_location import resolve_location_context, resolve_city_candidates, resolve_city_code
...
1. "[number] bus" or "[color][number]" -> route_name="[number]" (e.g., 307, 紅30)
2. "nearby bus", "bus stops" -> route_name="" (leave empty, use GPS)
3. Only fill city if explicitly mentioned (e.g. "Taipei 261 bus" -> route_name="261", city="Taipei"). City must be in English.
4. "how long until [number] arrives" -> route_name="[number]"
Always extract route_name if mentioned!"""
    CATEGORY = "道路運輸"
    ...
    def get_input_schema(cls) -> Dict[str, Any]:
        return StandardToolSchemas.create_input_schema({
            ...
            "location_query": {
                "type": "string",
                "description": "精確地址、地標、路口或站區（如「桃園火車站」「中正路口」）。提供時優先解析為座標做附近查詢"
            },
            "limit": {
                "type": "integer",
    ...
    async def execute(cls, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        location_query = str(arguments.get("location_query", "")).strip()
        ...
        location_ctx = await resolve_location_context(
            ...
            city_like=city_param,
            allowed_city_codes=cls.VALID_CITIES,
        )
        user_lat = location_ctx["lat"]
        user_lon = location_ctx["lon"]
        geo = location_ctx.get("geo") or {}
        city = resolve_city_code(city_param, allowed=cls.VALID_CITIES) or location_ctx["city_code"] or "Taipei"
        city_candidates = resolve_city_candidates(
            city_like=city_param or city,
            geo_city=geo.get("city"),
            geo_admin=geo.get("admin"),
            allowed_city_codes=cls.VALID_CITIES,
        )
        logger.debug(f"🏙️ [TDX] 城市候選: {city_candidates}, 主城市={city}")
        
        # 3. 執行查詢
        if route_name:
            return await cls._query_route_arrival(route_name, city, user_lat, user_lon, limit)
        else:
            return await cls._query_nearby_stops(user_lat, user_lon, city_candidates, limit)
    ...
    async def _query_route_arrival(
        ...
        try:
            logger.debug(f"🌐 [TDX] 開始並行查詢: {route_name}")
            eta_task = TDXBaseAPI.call_api(eta_endpoint, eta_params, cache_ttl=30)
            realtime_task = TDXBaseAPI.call_api(realtime_endpoint, realtime_params, cache_ttl=15)
            
            arrival_data, realtime_data = await asyncio.gather(eta_task, realtime_task, return_exceptions=True)
            
            if isinstance(arrival_data, Exception):
                raise arrival_data
            if isinstance(realtime_data, Exception):
                logger.warning(f"⚠️ 無法取得公車即時位置: {realtime_data}")
                realtime_data = []
                
            logger.debug(f"✅ [TDX] API 回應: {len(arrival_data) if arrival_data else 0} 筆到站資料")
            ...
            logger.error(f"❌ [TDX] 查詢異常: {e}")
            raise ExecutionError(f"查詢路線「{route_name}」時發生非預期錯誤")
        ...
        for bus in (realtime_data or []):
            ...
            event_type = bus.get("A2EventType", 0)
    ...
    async def _query_nearby_stops(
        ...
        for city in cities:
            ...
            city_stops = await TDXBaseAPI.call_api(endpoint, params, cache_ttl=1800)
            stops.extend(city_stops or [])
